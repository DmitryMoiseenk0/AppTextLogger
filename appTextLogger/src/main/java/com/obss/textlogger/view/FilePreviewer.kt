        ...
        view.let {
            ...
            binding.imageViewFilePreviewerSave.clickWithThrottle {
                ...
                coroutineCallLogger.launch {
                    async {
                        val fileDirectory =
                            Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)// activity.filesDir//это internal
                        val filePath: File
                        if (filePathName != null) {
                            ...
                            val format = SimpleDateFormat("dd_MM_yy_hh_mm_ss", Locale.US)
                            val name = format.format(Calendar.getInstance().time)
                            filePath = File(fileDirectory, "$TEXT_LOGGER ${name}.txt")
                            if (filePath.exists()) {
                                filePath.delete()
                            }
                        }
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            Toast.makeText(
                                activity,
                                resources.getString(
                                    R.string.file_previewer_save_success,
                                    filePath.path
                                ),
                                Toast.LENGTH_LONG
                            ).show()
